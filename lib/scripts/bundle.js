...
const createIndex = (icons, build) => (cb) => {
  const newfile = `${build}/index.tsx`;
  const newStory = `${build}/index.stories.tsx`;

  fs.writeFile(newfile, component.template('index.tsx', { icons }), 'utf-8', () => {
  ...
  fs.writeFile(newStory, component.template('stories.tsx', { icons }), 'utf-8', () => {
    const total = yellow.bold(Object.keys(icons).length + ' icons');
    const package = emoji.get(':package:');
    const message = `${package}  ${total} storybook bundled successfuly!`;
